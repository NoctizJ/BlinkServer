...
``read`` reports who is home and who is away, from state/presence.json::
    ...
    {}                  # everyone
    {"id": "娜"}        # just one person

Every read result carries a ready-to-display ``message`` — a formatted plain
text summary (see :func:`format_presence`) for putting on a dashboard, in a
notification, or in a terminal.
...
def read(payload: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    With no ``id`` it returns everyone's entry plus ``home``/``away`` name
    lists; with an ``id`` it returns just that person (``presence`` is null if
    they have never been seen). Either way ``message`` holds a formatted,
...
def write(payload: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    and ``event`` defaults to ``MANUAL_EVENT``.
    """
    ...
    try:
        entry = set_state(person, presence, event=str(event))
    except Exception as e:
        error_msg = str(e)
        logger.error("presence write failed for %s: %s", person, error_msg)
        write_log("blink", f"PRESENCE write ERROR ({person}): {error_msg}")
        return {
            "status": "error",
    ...
    write_log("blink", f"PRESENCE write: person={person} presence={presence} event={event}")
    return {"status": "ok", "id": person, "presence": entry}
